...
// ============================================
function formatSongMessage(title, content, type = 'info') {
    const emojis = {
        info: 'ℹ️',
        success: '✅',
        warning: '⚠️',
...
async function songCommand(sock, chatId, message) {
    try {
        ...
        if (!text) {
            ...
            const usageMsg = formatSongMessage(
                'SONG DOWNLOADER',
                `│ 🎵 *Usage:* .song <song name>\n│    or .song <YouTube link>\n│\n│ *Example:* .song shape of you`,
                'music'
            );
        ...
        if (text.includes('youtube.com') || text.includes('youtu.be')) {
			...
			if (!search || !search.videos.length) {
                ...
                const noResultsMsg = formatSongMessage(
                    'NO RESULTS',
        ...
        // Inform user with stylish format
        const infoMsg = formatSongMessage(
            'DOWNLOADING',
            `│ 🎵 *Title:* ${video.title}\n│ ⏱️ *Duration:* ${video.timestamp || 'Unknown'}\n│\n│ 📥 Please wait while I fetch your song...`,
            'download'
        );
		...
		// ENHANCEMENT: Success message (sent as caption with audio)
		const successCaption = `✨ *Download Complete!*\n\n🎵 *Title:* ${audioData.title || video.title || 'Song'}\n📥 *Format:* MP3\n\n𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗𝗘𝗗 𝗕𝗬 BATMAN MD

> *© ᴘᴏᴡᴇʀᴇᴅ ʙʏ ʙᴀᴛᴍᴀɴ ᴍᴅ*`;

		// Send buffer as MP3 with newsletter and caption
		await sock.sendMessage(chatId, {
			audio: finalBuffer,
        ...
        // Provide more specific error messages with stylish format
        let errorTitle = 'DOWNLOAD FAILED';
        let errorContent = '❌ Failed to download song.';
        ...
        const errorMsg = formatSongMessage(
            errorTitle,
            `│ ${errorContent}\n│ 🔧 ${err.message}`,
            'error'
        );
